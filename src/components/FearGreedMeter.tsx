...
interface IndexData {
  timestamp: string;
  currentIndex: number;
  indicators: Record<string, { value: number; weight: number }>;
}

// Helper functions for the gauge
...
export function FearGreedMeter() {
  ...
  const getSentimentColor = (value: number): string => {
    if (value <= 20) return "#c52828";  // Extreme Fear
    if (value <= 40) return "#e06c6c";  // Fear
    if (value <= 60) return "#f7c325";  // Neutral
    if (value <= 80) return "#72cc54";  // Greed
    return "#2aad27";  // Extreme Greed
  };
...
useEffect(() => {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
      {/* Top section with gauge and historical data */}
      <div style={{ 
        display: 'flex',
        gap: '40px',
        marginBottom: '40px'
      ...
      {/* Indicators explanation section */}
      <div style={{ 
